...
function PlotScatter(x::AbstractArray{T}, y::AbstractArray{T};
    ...
    LibCImPlot.PlotScatter(label, x, y, count, offset, stride * sizeof(T))
end
...
function PlotScatter(y::AbstractArray{T}; label::String="", count::Integer=length(y),
                  xscale::Real = 1.0, x0::Real = 0,
                  offset::Integer=0, stride::Integer=1) where {T<:ImPlotData}
    LibCImPlot.PlotScatter(label, y, count, xscale, x0, offset, stride * sizeof(T))
end
...
function PlotScatter(x::UnitRange{<:Integer}, y::AbstractArray{T};
    ...
    LibCImPlot.PlotScatter(label, y, count, xscale, x0, offset, stride)
end
