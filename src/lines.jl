...
function PlotLine(x::AbstractArray{T}, y::AbstractArray{T};
    ...
    LibCImPlot.PlotLine(label, x, y, count, offset, stride * sizeof(T))
end
...
function PlotLine(y::AbstractArray{T}; label::String="", count::Integer=length(y),
                  xscale::Real = 1.0, x0::Real = 0.0, offset::Integer=0,
                  stride::Integer=1) where {T<:ImPlotData}
    LibCImPlot.PlotLine(label, y, count, xscale, x0, offset, stride * sizeof(T))
end

function PlotLine(x::UnitRange{<:Integer}, y::AbstractArray{T}; xscale::Real = 1.0,
                  x0::Real = 0.0, label::String="") where {T<:ImPlotData}
    ...
    LibCImPlot.PlotLine(label, y, count,  xscale, x0, offset, stride)
end

function PlotLine(x::StepRange, y::AbstractArray{T}; xscale::Real = 1.0, x0::Real = 0.0,
                  label::String= "") where {T<:ImPlotData}
